    ...
    <article className="rounded-lg border border-gray-700 bg-panel p-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-300">{props.type} · par {props.creatorName} · {props.createdAt}</p>
        </div>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {props.tags.map((tag) => (
          <span key={tag} className="rounded-full border border-gray-600 px-2 py-1 text-xs text-gray-300">#{tag}</span>
        ))}
      </div>
      <div className="mt-4 flex items-center gap-4 text-sm">
        <a href={props.sourceUrl} target="_blank" rel="noreferrer">Source</a>
